...
logger = optim.Logger('train_loss_8.log')
...
local optimState = {
	learningRate = 0.0000000001, -- learning rate 10^-4 as per the paper
...
fcn_net = torch.load('../Model/fcn8_1_130.t7')
-- fcn_net = torch.load('./pre_fcn8.t7')
...
function train()
	...
	for epoch = 131, config.max_epoch do
		...
		if epoch%10 == 0 then
			torch.save('../Model/fcn8_1_' .. epoch .. '.t7', fcn_net)
		end
	end
	-- logger:plot()
end
