...
logger = optim.Logger('train_loss_32.log')
...
local optimState = {
	learningRate = 0.00005, -- learning rate 10^-4 as per the paper
...
function train()
	...
	for epoch = 51, config.max_epoch do
		...
		if epoch%25 == 0 then
			torch.save('fcn32_' .. model_idx .. '.t7', fcn_net)
